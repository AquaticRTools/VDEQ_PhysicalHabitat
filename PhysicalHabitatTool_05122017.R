...
# LAST UPDATED: 05/12/2017 by Emma Jones (emma.jones@deq.virginia.gov)
...
setwd("C:/HardDriveBackup/PHAB_R")

# Connect to Access Database and read in Access tables
channel <- odbcConnectAccess("PHab_04_17_2017.mdb")
...
substrate_summary.alt <- join(substratePCT,substrate_summary[,c(1:3,22,23)],by=c('SampleID','StationID','Date'))
...
IN_substrate_summary.alt <- join(IN_substratePCT,IN_substrate_summary[,c(1:3,22,23)],by=c('SampleID','StationID','Date'))
...
OUT_substrate_summary.alt <- join(OUT_substratePCT,OUT_substrate_summary[,c(1:3,22,23)]
...
TMDLSummary <- mutate(summary,radiusBKF=((Xdepth/100)+XBKF_H)/2,Dcbf=13.7*radiusBKF*Slope,RBS1=SUB_DMM/Dcbf
                       ...
                       ,LDMB=log10(D_cbf),LRBS2=LSUB_DMM-LDMB,BKF_depth_in_meters=(Xdepth/100)+XBKF_H
  ...
  select(SampleID,StationID,Date,SiteFlag,ThalwegN,ReachLength,Interval,Slope,RP100,BR_PCT,HP_PCT,RC_PCT
         ...
         ,LSUB_DMM,VLW_msq,Xdepth,Xwid,BKF_depth_in_meters,BKFW_BKFD,incised_depth,Xembed
...
MasterSummary <- mutate(summary,radiusBKF=((Xdepth/100)+XBKF_H)/2,Dcbf=13.7*radiusBKF*Slope
                        ...
                        ,LRBS2=LSUB_DMM-LDMB,BKF_depth_in_meters=(Xdepth/100)+XBKF_H
                        ,BKFW_BKFD=XBKF_W/BKF_depth_in_meters,incised_depth=(Xdepth/100)+INC_H
...
MasterSummary <- select(MasterSummary,SampleID,StationID,Date,SiteFlag,ThalwegN,ReachLength,Interval
                        ...
                        ,LRBS1,RW,RP,R_bf,D_cbf,LDMB,LRBS2,BKF_depth_in_meters,BKFW_BKFD,incised_depth
